...
% Last Modified by GUIDE v2.5 11-Nov-2020 17:55:52
...
for i = 1:handles.nrofiterations
 ...
 hiddenlayerlistlength = hiddenlayerlistlength + 1;
 for k = 1:handles.nrofwordnethiddenunits
  hiddenlayerlist(hiddenlayerlistlength,k) = handles.wordnethiddenunits(k);
 end
...
% Reduce dimensionality of the state layer list for display
if handles.trajectories2d == 1
 [hiddenlayerlist2d, eigvect, eigval, variances] = pca(hiddenlayerlist, 0, 2);
end
if handles.trajectories3d == 1
 [hiddenlayerlist3d, eigvect, eigval, variances] = pca(hiddenlayerlist, 0, 3);
end;

 % Set up a plot window to display reduced-dimensionality state layers
if or(handles.trajectories2d == 1, handles.trajectories3d == 1)
 figure;
 axes;
 axis ([-1 1 -1 1 -1 1]);
 axis equal;
...
dlmwrite('hiddenlayerlist.txt',hiddenlayerlist, 'delimiter', '\t', 'precision','%.3f', 'newline', 'pc');
...
% Reduce dimensionality
 ...
 for i = 1:handles.nrofwords
  ...
  currentwordlength = length(currentword);
  counter = counter + 1;
  vfrom = hiddenlayerlist2d(counter,:);
  scatter (vfrom(1), vfrom(2), 'b', '+');
  hold on
  for j = 2:currentwordlength
   counter = counter + 1;
   vto = hiddenlayerlist2d(counter,:);
...
if handles.trajectories3d == 1
 ...
 for i = 1:handles.nrofwords
  ...
  currentwordlength = length(currentword);
  counter = counter + 1;
  vfrom = hiddenlayerlist3d(counter,:);
  scatter3 (vfrom(1), vfrom(2), vfrom(3), 'b', '+');
  hold on
  for j = 2:currentwordlength
   counter = counter + 1;
   vto = hiddenlayerlist3d(counter,:);
   if j == currentwordlength
