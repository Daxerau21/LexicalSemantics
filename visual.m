...
for i = 1:handles.nrofiterations
 ...
 for j = 1:handles.nrofcycles
  ...
  for j = 1:handles.nrofvisualnethiddenunits
   handles.hiddenlayerlist(handles.hiddenlayerlistlength, j) = handles.visualnethiddenunits(j);   
  end
...
for i = 1:handles.nrofvisualcodes
 ...
 % Save hidden layer
 for j = 1:handles.nrofvisualnethiddenunits
  handles.testhiddenlayerlist(i,j) = handles.visualnethiddenunits(j);
 end
end
...
if handles.trajectories2d == 1
  ...
  [testhiddenlayerlist2d, eigvect, eigval, variances] = pca(handles.testhiddenlayerlist, 0, 2);
 ...
 if handles.trajectories3d == 1
  ...
  % Reduce dimensionality of test hidden layers
  [testhiddenlayerlist3d, eigvect, eigval, variances] = pca(handles.testhiddenlayerlist, 0, 3);
...
disp('Representations complete')
% Write visual representations to file
dlmwrite('visualreps.txt',handles.testhiddenlayerlist, 'delimiter', '\t', 'precision','%.3f', 'newline', 'pc');
guidata (gcbo, handles);
